...
device = 'cuda'
dataset_path = '2025-06-13/Multiclass/KDD_Cup_1999_Multiclass.parquet'
target_column = 'label'
...
# print(f'Original  DF memory usage: {df_full.memory_usage(deep=True).sum() / 1024 / 1024:.2f} MB')

# dtypes_before = df_full.dtypes.apply(lambda x: x.name).to_dict()
# df_report = report_on_dataframe(df_full, unit="MB", optimize="computation")
# df_full = optimize_dtypes(df_full, df_report)
# for col in df_full.select_dtypes(include=['integer', 'float']).columns:
#     df_full[col] = pd.to_numeric(df_full[col], downcast='integer' if df_full[col].dtype.kind == 'i' else 'float')
# dtypes_after = df_full.dtypes.apply(lambda x: x.name).to_dict()

# print(f'Optimized DF memory usage: {df_full.memory_usage(deep=True).sum() / 1024 / 1024:.2f} MB')
...
# Store mapping: column -> list of categories

# Build mapping: column name → list of categories
# category_mappings = {
#     col: {int(i): cat for i, cat in enumerate(df_full[col].cat.categories)}
#     for col in categorical_cols
# }

# Apply .cat.codes
# df_full[categorical_cols] = df_full[categorical_cols].apply(lambda col: col.cat.codes)
...
# metadata = {
#     'features': {
#         'categorical_cols': {k: str(v) for k, v in df_full[categorical_cols].dtypes.to_dict().items()},
#         'numerical_cols': {
#             k: str(v) for k, v in df_full
#             .select_dtypes(include=['number'])
#             .drop(columns=[*categorical_cols, target_column], errors='ignore')
...
#     'target': {
#         'name': target_column,
#         'unique_values': unique_values.categories.tolist(),
#         'dtype': str(df_full[target_column].dtype),
#         'label_to_index': {str(k): int(v) for k, v in label_to_index.items()},
#         'index_to_label': {int(k): str(v) for k, v in index_to_label.items()},
#         'value_counts': df_full[target_column].value_counts().to_dict()
#     }
# }

# pprint(metadata, indent=4)

# metadata_filename = 'deploy/multiclass/metadata.json'
...
# pd.DataFrame({
#     "column": list(dtypes_before.keys()),
#     "dtype_before": list(dtypes_before.values()),
#     "dtype_after": [dtypes_after[col] for col in dtypes_before.keys()]
# })
...
def assign_subsets(df, stratify_col, train_frac, val_frac, test_frac, random_state):
    ...
    if device == 'cpu':
        ans = pd.concat([df_train, df_val, df_test]).reset_index(drop=True)
    else:
        ans = cudf.concat([df_train, df_val, df_test]).reset_index(drop=True)
...
if device == 'cpu':
    assert set(X_train_full.index).isdisjoint(set(X_val_full.index)), "Train and Validation sets are not disjoint!"
    assert set(X_train_full.index).isdisjoint(set(X_test_full.index)), "Train and Test sets are not disjoint!"
    assert set(X_val_full.index).isdisjoint(set(X_test_full.index)), "Validation and Test sets are not disjoint!"
else:
...
print(f"Train      : {len(df_train_full)} samples ({(100.0 * len(df_train_full) / len(df_full)):.2f}%), {y_train_full.nunique()} unique classes ({sorted(y_train_full.unique().tolist())})")
print(f"Validation : {len(df_val_full)} samples ({(100.0 * len(df_val_full) / len(df_full)):.2f}%), {y_val_full.nunique()} unique classes ({sorted(y_train_full.unique().tolist())})")
print(f"Test       : {len(df_test_full)} samples ({(100.0 * len(df_test_full) / len(df_full)):.2f}%), {y_test_full.nunique()} unique classes ({sorted(y_train_full.unique().tolist())})")
...
def train_xgb(
    ...
    if cv:
        ...
        if device == 'cpu':
            f1_score_ans = float(f1_score(y_test, y_pred, average="weighted"))
        else:
...
# Ensure disjoint splits
if device == 'cpu':
    assert set(X_train_sampled.index).isdisjoint(set(X_val_sampled.index)), "Train and Validation sets are not disjoint!"
    assert set(X_train_sampled.index).isdisjoint(set(X_test_sampled.index)), "Train and Test sets are not disjoint!"
    assert set(X_val_sampled.index).isdisjoint(set(X_test_sampled.index)), "Validation and Test sets are not disjoint!"
else:
...
# Print final distributions
print(f"Train      : {len(df_train_sampled)} samples ({(100.0 * len(df_train_sampled) / len(df_full)):.2f}%), {y_train_sampled.nunique()} unique classes ({sorted(y_train_sampled.unique().tolist())})")
print(f"Validation : {len(df_val_sampled)} samples ({(100.0 * len(df_val_sampled) / len(df_full)):.2f}%), {y_val_sampled.nunique()} unique classes ({sorted(y_train_sampled.unique().tolist())})")
print(f"Test       : {len(df_test_sampled)} samples ({(100.0 * len(df_test_sampled) / len(df_full)):.2f}%), {y_test_sampled.nunique()} unique classes ({sorted(y_train_sampled.unique().tolist())})")
...
# Select top features
selected_features = importance_df["Feature"][:N].values.tolist()
...
def convert_categorical_to_frequency(df, normalize=True):
    ...
    for col in df_encoded.select_dtypes(include=['category']).columns:
        ...
        row_mappings[col] = df[col]  # row-wise original categories
...
def make_numeric_scaler(scaling_method, X):
    ...
    def scale_numeric_features(X_input, fit=False):
        ...
        if scaling_method == 'none':
            ...
            if device == 'cpu':
                X_numeric_scaled_pd = pd.DataFrame(
                    X_numeric_scaled_pd, columns=numeric_columns, index=X_input.index
                ).astype('float32')
                X_output = pd.concat([X_numeric_scaled_pd, X_non_numeric_pd], axis=1)
            elif device == 'cuda':
                import cudf
...
if device == 'cpu':
    from category_encoders import OneHotEncoder as CEOneHotEncoder
...
def make_categorical_encoder(encoding_method, X, max_categories_for_onehot=10):
    ...
    encoders = {
        ...
        'onehot': CumlOneHotEncoder(handle_unknown='ignore', sparse=False, sparse_output=False)
                  if device == 'cuda' else CEOneHotEncoder(cols=onehot_columns, use_cat_names=True, handle_unknown='value'),
        'ordinal': CumlLabelEncoder(handle_unknown='ignore')
                   if device == 'cuda' else CEOrdinalEncoder(cols=categorical_columns, handle_unknown='value', return_df=True),        
    ...
    def encode_categorical_features(X_input, fit=False):
        ...
        if encoding_method == 'none':
            ...
            if device == 'cuda':
                ...
                # CPU version using category_encoders
                if fit:
                    encoder.fit(X_categorical[onehot_columns])
                X_onehot = encoder.transform(X_categorical[onehot_columns]).astype(int)
                # Ensure index alignment
                X_onehot.index = X_numeric_copy.index
                X_encoded = pd.concat([X_numeric_copy, X_onehot], axis=1)
            ...
            if device == 'cuda':
                ...
                X_ordinal = encoder.transform(X_categorical)
                X_encoded = pd.concat([X_numeric_copy, X_ordinal], axis=1)
            return X_encoded
            ...
            if device == 'cpu':
                return pd.concat([X_numeric_copy, X_categorical_encoded], axis=1)
            else:
...
def fit_resample(_X_train, _y_train, over_method, over_thresh, under_method, under_thresh):
    ...
    if over_thresh:
        ...
        # if device == 'cuda' and isinstance(_X_train_copy, cudf.DataFrame):
        #     _X_train_copy, _y_train_copy = over_sampler.fit_resample(_X_train_copy.to_pandas(), _y_train_copy)
        # else:
    ...
    if under_thresh:
        ...
        # if device == 'cuda' and isinstance(_X_train_copy, cudf.DataFrame):
        #     _X_train_copy, _y_train_copy = under_sampler.fit_resample(_X_train_copy.to_pandas(), _y_train_copy)
        # else:
...
from cudf_resampler import CuDFResampler
...
def make_over_sampler(over_method, over_strategy, n_neighbors, cat_features=None):
    if over_method == "random":
        if device == "cpu":
            return RandomOverSampler(random_state=random_state, sampling_strategy=over_strategy)
        else:
            return CuDFResampler(strategy=over_strategy, sampling_type="over")
    elif over_method == "smote":
        if device == "cpu":
            return SMOTE(k_neighbors=SklearnNearestNeighbors(n_neighbors=n_neighbors), sampling_strategy=over_strategy)
        else:
        ...
        if device == "cpu":
            return SMOTENC(categorical_features=cat_features, k_neighbors=SklearnNearestNeighbors(n_neighbors=n_neighbors), sampling_strategy=over_strategy)
        else:
...
def make_under_sampler(under_method, under_strategy):
    if under_method == "random":
        if device == 'cpu':
            return RandomUnderSampler(random_state=random_state, sampling_strategy=under_strategy)
        else:
            return CuDFResampler(strategy=under_strategy, sampling_type="under")
    elif under_method == "tomek":
        if device == 'cpu':
            return TomekLinksImblearn(n_jobs=n_jobs)
        elif device == 'cuda':
...
def objective(trial):
    try:
        ...
        assert isinstance(X_val_reduced, cudf.DataFrame)

        scaling_method = trial.suggest_categorical('scaling_method', scaling_methods)
        scaler = make_numeric_scaler(scaling_method, X_train_filtered)

        X_train_scaled = scaler(X_train_filtered, fit=True)
        X_val_scaled = scaler(X_val_reduced)

        assert X_train_scaled.shape == X_train_filtered.shape
        assert X_val_scaled.shape == X_val_reduced.shape
        assert X_train_scaled.isna().sum().sum() == 0
        assert X_val_scaled.isna().sum().sum() == 0

        # Apply Categorical Encoding

        assert isinstance(X_train_scaled, cudf.DataFrame)
        assert isinstance(X_val_scaled, cudf.DataFrame)

        encoding_method = trial.suggest_categorical('encoding_method', encoding_methods)
        encoder = make_categorical_encoder(encoding_method, X_train_scaled)

        X_train_encoded = encoder(X_train_scaled, fit=True)
        X_val_encoded = encoder(X_val_scaled)

        # Some train feature categories might not be present in validation
        # (and vice-versa); in that case, we add a new "missing" category
        fill_categorical_nas([X_train_encoded, X_val_encoded])

        assert X_train_encoded.shape[0] == X_train_scaled.shape[0]
        assert X_train_encoded.shape[1] >= X_train_scaled.shape[1]
        assert X_train_encoded.isna().sum().sum() == 0
        assert X_val_encoded.isna().sum().sum() == 0

        # Apply Data Balancing

        assert isinstance(X_train_encoded, cudf.DataFrame)
        ...
        X_dtypes_before = X_train_encoded.dtypes.copy(deep=True).sort_index()
        y_dtype_before = y_train_filtered.dtype

        X_train_resampled, y_train_resampled = fit_resample(
            X_train_encoded, y_train_filtered, over_method, over_threshold, under_method, under_threshold
        )
        ...
        assert X_train_resampled.shape[1] == X_train_encoded.shape[1]
        ...
        hpo_booster_params = {
            ...
            "max_depth": trial.suggest_int("max_depth", 3, 10),                    # Maximum depth of trees, prevents overfitting
        ...
        model_hpo, train_time_hpo, latency_hpo, f1_weighted_hpo, model_size_hpo = \
            train_xgb(
                X_train_resampled, None, X_val_encoded,   # No validation set since CV == True
        ...
        raise optuna.TrialPruned(
            f'Scaling_Method: {trial.params["scaling_method"]} | Encoding_Method: {trial.params["encoding_method"]} | '
...
col_order = ['number', 'datetime_start', 'datetime_complete', 'duration', 'latency', 'f1_score',
             "scaling_method", "encoding_method", "over_method", "over_threshold", "under_method", "under_threshold",
...
df_hpo_styled.to_excel(f"output/nc={len(labels)}_sr={sampling_rate_sets:.2f}_{device}_trials.xlsx")
with open(f"output/nc={len(labels)}_sr={sampling_rate_sets:.2f}_{device}_trials.html", "w", encoding="utf-8") as f:
...
fig.write_html(f"output/nc={len(labels)}_sr={sampling_rate_sets:.2f}_{device}_hpo_optimization_history_1_latency.html", full_html=True)
# fig.show()
...
fig = go.Figure(fig)
fig.update_layout(width=None, height=None, autosize=True)
fig.write_html(f"output/nc={len(labels)}_sr={sampling_rate_sets:.2f}_{device}_hpo_optimization_history_2_score.html", full_html=True)
# fig.show()
...
# Save and show plot
fig.write_html(f"output/nc={len(labels)}_sr={sampling_rate_sets:.2f}_{device}_hpo_optimization_history_3_balance.html", full_html=True)
# fig.show()
...
fig = go.Figure(fig)
fig.update_layout(width=None, height=None, autosize=True)
fig.write_html(f"output/nc={len(labels)}_sr={sampling_rate_sets:.2f}_{device}_hpo_parallel_coordinate_1_latency.html", full_html=True)
# fig.show()
...
fig = go.Figure(fig)
fig.update_layout(width=None, height=None, autosize=True)
fig.write_html(f"output/nc={len(labels)}_sr={sampling_rate_sets:.2f}_{device}_hpo_parallel_coordinate_2_score.html", full_html=True)
# fig.show()
...
if plot_param_importances:
    ...
    # Save and show plot
    fig.write_html(f"output/nc={len(labels)}_sr={sampling_rate_sets:.2f}_{device}_hpo_param_importances.html", full_html=True)
    # fig.show()
...
def reconstruct_and_evaluate_best_model(best_params, optimized_metric, persist: bool = True, filename: str = "deploy/multiclass/xgb_model.json"):
    ...
    # Redo preprocessing (Separate Scaling & Encoding)
    best_scaling_method = best_params_copy.pop('scaling_method')
    best_encoding_method = best_params_copy.pop('encoding_method')

    scaler = make_numeric_scaler(best_scaling_method, X_train_filtered)
    encoder = make_categorical_encoder(best_encoding_method, X_train_filtered)

    X_train_scaled = scaler(X_train_filtered, fit=True)
    X_val_scaled = scaler(X_val_reduced)
    X_test_scaled = scaler(X_test_reduced)

    X_train_encoded = encoder(X_train_scaled, fit=True)
    X_val_encoded = encoder(X_val_scaled)
    X_test_encoded = encoder(X_test_scaled)
    ...
    X_train_resampled, y_train_resampled = fit_resample(
        X_train_encoded, y_train_filtered, best_over_method, best_over_threshold, best_under_method, best_under_threshold
    )
    ...
    model_best, train_time_best, latency_best, f1_weighted_best, model_size_best = train_xgb(
        X_train_resampled, X_val_encoded, X_test_encoded,
...
summary_df.to_excel(f"output/nc={len(labels)}_sr={sampling_rate_sets:.2f}_{device}_summary_table.xlsx")
with open(f"output/nc={len(labels)}_sr={sampling_rate_sets:.2f}_{device}_summary_table.html", "w", encoding="utf-8") as f:
