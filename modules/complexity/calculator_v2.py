...
from modules.complexity.preparer_v2 import get_folder_size, now, safe_call
...
if __name__ == "__main__":
    ...
    ds_folders = sorted(
        ...
        key=lambda f: get_folder_size(os.path.join(BASE_FOLDER, f))
    )
        ...
        for name in tqdm(ds_folders, desc='Dataset ', leave=False):
            ...
            for seed in tqdm(SEEDS, desc='Seed    ', leave=False):
                ...
                for frac in tqdm(SAMPLE_FRACS, desc='Fraction', leave=False):
                    ...
                    dst_path = Path(str(abs_path).replace('.npz', f'.complexity.json'))
                    src_path_short = '/'.join(str(src_path).split('/')[-4:])
                    src_size = Path(src_path).stat().st_size / 1024 / 1024
                    ...
                    try:
                        ...
                        debug_json_path = Path(
                            str(dst_path).replace(
                                ".complexity.json",
                                ".complexity.error" if metrics["errors"] else ".complexity.success"
                            )
                        )
                        ...
                        # Ensure thread is stopped even if iteration fails
                        monitor.stop(stop_pynvml=False)

    finally:
        # This code runs no matter what: 
        # 1. After all loops finish naturally
        # 2. If you press Ctrl+C
        # 3. If there is an unhandled crash
        tqdm.write(f"[{now()}] Cleaning up resources...")
        monitor.stop(stop_pynvml=True)