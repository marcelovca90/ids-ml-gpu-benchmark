...
def load_csv(dataset_folder, filename):
    ...
    full_filename = os.path.join(base_folder, filename)
    df = pd.read_csv(
        filepath_or_buffer=full_filename).infer_objects().to_numpy()
    return df.ravel() if df.shape[1] == 1 else df


def load_ftr(dataset_folder, filename):
    base_folder = os.path.join(os.path.dirname(__file__), dataset_folder)
    full_filename = os.path.join(base_folder, filename)
    df = pd.read_feather(path=full_filename).infer_objects().to_numpy()
    return df.ravel() if df.shape[1] == 1 else df
...
def sort_results(best_results):
    ...
    for key, value in best_results.items():
        ...
        try:
            best_results[key] = sorted(
                [v for v in best_results[key]], key=lambda d: d['user_attrs']['f1_score_weighted'], reverse=True)
...
def persist_best_results(file_prefix, uuid, best_results):
    with open(os.path.join(file_prefix, f'{uuid}.json'), 'w') as fp:
...
def plot_best_and_mean_results(file_prefix, uuid, best_results):
    ...
    plt.ylim(0, 1.05)
    plt.savefig(os.path.join(file_prefix, f'{uuid}.png'))
