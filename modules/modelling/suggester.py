...
def get_baseline_suggestion(X_train, y_train, classifier_name, trial):
    ...
    if classifier_name == 'AdaBoostClassifier':
        ...
        classifier_obj = BalancedBaggingClassifier(
            random_state=SEED, n_jobs=N_JOBS_CLF)

    elif classifier_name == 'BalancedRandomForestClassifier':
        classifier_obj = BalancedRandomForestClassifier(
            random_state=SEED, n_jobs=N_JOBS_CLF)
        ...
        classifier_obj = CatBoostClassifier(
            random_state=SEED, thread_count=N_JOBS_CLF)
        ...
        classifier_obj = EasyEnsembleClassifier(
            random_state=SEED, n_jobs=N_JOBS_CLF)
        ...
        classifier_obj = KNeighborsClassifier(n_jobs=N_JOBS_CLF)

    elif classifier_name == 'LGBMClassifier':
        classifier_obj = LGBMClassifier(random_state=SEED, n_jobs=N_JOBS_CLF)
        ...
        classifier_obj = make_pipeline(
            AdditiveChi2Sampler(), LinearSVC(random_state=SEED))

    elif classifier_name == 'LinearSVC_Nystroem':
        classifier_obj = make_pipeline(
            Nystroem(random_state=SEED), LinearSVC(random_state=SEED))

    elif classifier_name == 'LinearSVC_PolynomialCountSketch':
        classifier_obj = make_pipeline(PolynomialCountSketch(
            random_state=SEED), LinearSVC(random_state=SEED))

    elif classifier_name == 'LinearSVC_RBFSampler':
        classifier_obj = make_pipeline(RBFSampler(
            random_state=SEED), LinearSVC(random_state=SEED))

    elif classifier_name == 'LogisticRegression':
        classifier_obj = LogisticRegression(n_jobs=N_JOBS_CLF)
        ...
        classifier_obj = PassiveAggressiveClassifier(
            random_state=SEED, n_jobs=N_JOBS_CLF)

    elif classifier_name == 'Perceptron':
        classifier_obj = Perceptron(random_state=SEED, n_jobs=N_JOBS_CLF)
        ...
        classifier_obj = RadiusNeighborsClassifier(
            random_state=SEED, n_jobs=N_JOBS_CLF)
        ...
        classifier_obj = SGDClassifier(random_state=SEED, n_jobs=N_JOBS_CLF)
        ...
        classifier_obj = XGBClassifier(seed=SEED, n_jobs=N_JOBS_CLF)

    # return OneVsRestClassifier(classifier_obj, n_jobs=N_JOBS_OVR)
...
def get_optimized_suggestion(X_train, y_train, classifier_name, trial):
    ...
    if classifier_name == 'AdaBoostClassifier':
        ...
        l1_ratio = trial.suggest_discrete_uniform(
            'agc_l1_ratio', 0.0, 1.0, 0.1)
        loss = trial.suggest_categorical('agc_loss', [
                                         'modified_huber', 'hinge', 'smooth_hinge', 'squared_hinge', 'perceptron', 'log', 'squared'])
        ...
        max_features = trial.suggest_int(
            'brf_max_features', 1, X_train.shape[1])
        ...
        binarize = trial.suggest_discrete_uniform(
            'bnb_binarize', 0.0, 1.0, 0.05)
        ...
        learningMethod = trial.suggest_categorical(
            'bnc_learningMethod', ["Chow-Liu", "NaiveBayes", "GHC", "MIIC", "TAN", "Tabu"])
        prior = trial.suggest_categorical(
            'bnc_prior', ["Smoothing", "BDeu", "Dirichlet", "NoPrior"])
        scoringType = trial.suggest_categorical(
            'bnc_scoringType', ["AIC", "BIC", "BD", "BDeu", "K2", "Log2"])
        discretizationStrategy = trial.suggest_categorical('bnc_discretizationStrategy', [
                                                           'quantile', 'uniform', 'kmeans', 'NML', 'CAIM', 'MDLP'])
        discretizationNbBins = trial.suggest_int(
            'bnc_discretizationNbBins', 10, 50, 5)
        ...
        learning_rate = trial.suggest_loguniform(
            'cbc_learning_rate', 1e-3, 1e0)
        sampling_frequency = trial.suggest_categorical(
            'cbc_sampling_frequency', ['PerTree', 'PerTreeLevel'])
        ...
        loss = trial.suggest_categorical(
            'cdc_loss', ['squared_hinge', 'log', 'modified_huber', 'squared'])
        penalty = trial.suggest_categorical(
            'cdc_penalty', ['l1', 'l2', 'l1/l2'])
        ...
        fit_prior = trial.suggest_categorical(
            'compnb_fit_prior', [False, True])
        norm = trial.suggest_categorical('compnb_norm', [False, True])
        feature_scaler = MinMaxScaler()
        classifier_tmp = ComplementNB(alpha=alpha,
                                      fit_prior=fit_prior,
                                      norm=norm)
        classifier_obj = make_pipeline(feature_scaler, classifier_tmp)
        ...
        max_features = trial.suggest_int(
            'dtc_max_features', 1, X_train.shape[1])
        ...
        sampling_strategy = trial.suggest_categorical(
            'eec_sampling_strategy', ['majority', 'not minority', 'not majority', 'all'])
        ...
        learning_iterations = trial.suggest_categorical(
            'elcs_learning_iterations', [2000])
        ...
        max_features = trial.suggest_int(
            'etc_max_features', 1, X_train.shape[1])
        ...
        max_features = trial.suggest_int(
            'etsc_max_features', 1, X_train.shape[1])
        ...
        var_smoothing = trial.suggest_loguniform(
            'gnb_var_smoothing', 1e-12, 1e3)
        classifier_obj = GaussianNB(var_smoothing=var_smoothing)

    elif classifier_name == 'GaussianProcessClassifier':
        max_iter_predict = trial.suggest_int(
            'gpc_max_iter_predict', 50, 200, 10)
        multi_class = trial.suggest_categorical(
            'gpc_multi_class', ['one_vs_one', 'one_vs_rest'])
        ...
        metric = trial.suggest_categorical('knc_metric', [
                                           'braycurtis', 'canberra', 'chebyshev', 'cityblock', 'correlation', 'cosine', 'mahalanobis'])
        ...
        n_components = trial.suggest_int(
            'lda_n_components', 1, min(n_features, n_classes-1), 1)
        tol = 1e-4 if solver == 'svd' else trial.suggest_categorical('lda_tol', [
                                                                     1e-5, 1e-4, 1e-3])
        ...
        kernel_sample_steps = trial.suggest_categorical(
            'lsvc_ac2s_kernel_sample_steps', [1, 2, 3])
        ...
        kernel_gamma = trial.suggest_loguniform(
            'lsvc_nystroem_kernel_gamma', 1e-3, 1e3)
        kernel_n_components = trial.suggest_int(
            'lsvc_nystroem_kernel_n_components', 50, 150, 50)
        ...
        kernel_gamma = trial.suggest_loguniform(
            'lsvc_pcs_kernel_gamma', 1e-3, 1e3)
        kernel_n_components = trial.suggest_int(
            'lsvc_pcs_kernel_n_components', 50, 150, 50)
        ...
        kernel_gamma = trial.suggest_loguniform(
            'lsvc_rbfs_kernel_gamma', 1e-3, 1e3)
        kernel_n_components = trial.suggest_int(
            'lsvc_rbfs_kernel_n_components', 50, 150, 50)
        ...
        def create_hidden_layers(value, count): return tuple(
            [int(value*2**(count-i-1)) for i in range(0, count)])
        hidden_layer_count = trial.suggest_int(
            'mlpc_hidden_layer_count', 1, 3, 1)
        hidden_layer_sizes = create_hidden_layers(
            2*X_train.shape[1], hidden_layer_count)
        learning_rate = trial.suggest_categorical(
            'mlpc_learning_rate', ['constant', 'invscaling', 'adaptive'])
        learning_rate_init = trial.suggest_loguniform(
            'mlpc_learning_rate_init', 1e-6, 1e0)
        ...
        metric = trial.suggest_categorical(
            'nc_metric', ['cityblock', 'cosine', 'euclidean', 'l1', 'l2', 'manhattan'])
        shrink_threshold = trial.suggest_categorical(
            'nc_shrink_threshold', [None, 0.2])
        ...
        coef0 = trial.suggest_discrete_uniform(
            'nusvcpoly_coef0', 0.0, 1.0, 0.05)
        ...
        early_stopping = trial.suggest_categorical(
            'pac_early_stopping', [True])
        validation_fraction = trial.suggest_categorical(
            'pac_validation_fraction', [0.25])
        n_iter_no_change = trial.suggest_categorical(
            'pac_n_iter_no_change', [10])
        ...
        reg_param = trial.suggest_discrete_uniform(
            'qda_reg_param', 0.0, 1.0, 0.05)
        ...
        max_features = trial.suggest_int(
            'rfc_max_features', 1, X_train.shape[1])
        ...
        learning_rate = trial.suggest_loguniform(
            'rusbc_learning_rate', 1e-6, 1e0)
        sampling_strategy = trial.suggest_categorical(
            'rusbc_sampling_strategy', ['majority', 'not minority', 'not majority', 'all'])
        ...
        loss = trial.suggest_categorical(
            'sgdc_loss', ['hinge', 'log', 'modified_huber', 'squared_hinge'])
        penalty = trial.suggest_categorical('sgdc_penalty', ['elasticnet'])
        alpha = trial.suggest_loguniform('sgdc_alpha', 1e-6, 1e3)
        l1_ratio = trial.suggest_discrete_uniform(
            'sgdc_l1_ratio', 0.0, 1.0, 0.2)
        learning_rate = trial.suggest_categorical(
            'sgdc_learning_rate', ['constant', 'optimal', 'invscaling', 'adaptive'])
        eta0 = trial.suggest_loguniform('sgdc_eta0', 1e-3, 1e3)
        early_stopping = trial.suggest_categorical(
            'sgdc_early_stopping', [True])
        validation_fraction = trial.suggest_categorical(
            'sgdc_validation_fraction', [0.25])
        n_iter_no_change = trial.suggest_categorical(
            'sgdc_n_iter_no_change', [10])
        ...
        lambda_sparse = trial.suggest_loguniform(
            'tnc_lambda_sparse', 1e-6, 1e0)
        ...
        ordered_rules = trial.suggest_categorical(
            'vfdrc_ordered_rules', [False, True])
        grace_period = trial.suggest_int('vfdrc_grace_period', 100, 400, 50)
        tie_threshold = trial.suggest_discrete_uniform(
            'vfdrc_tie_threshold', 0.0, 1.0, 0.05)
        rule_prediction = trial.suggest_categorical(
            'vfdrc_rule_prediction', ['first_hit', 'weighted_max', 'weighted_sum'])
        max_rules = trial.suggest_int('vfdrc_max_rules', 10, 40, 5)
        expand_criterion = trial.suggest_categorical(
            'vfdrc_expand_criterion', ['info_gain', 'hellinger', 'foil_gain'])
        remove_poor_atts = trial.suggest_categorical(
            'vfdrc_remove_poor_atts', [False, True])
        ...
        use_label_encoder = trial.suggest_categorical(
            'xgbc_use_label_encoder', [False])
        learning_rate = trial.suggest_loguniform(
            'xgbc_learning_rate', 1e-6, 1e0)
        booster = trial.suggest_categorical(
            'xgbc_booster', ['gbtree', 'gblinear', 'dart'])
