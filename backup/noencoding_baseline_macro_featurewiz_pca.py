...
from study_utils import (calculate_score, load_csv, load_ftr, load_mappings,
                         persist_best_results, plot_best_and_mean_results,
                         sort_results, truncate)

QUICK_RUN      = False
FILE_PREFIX    = 'results/bot_iot/macro/featurewiz_pca/baseline'
DATASET_FOLDER = '../datasets/bot_iot/generated/macro/featurewiz_pca/'
...
X_train, X_test = load_ftr(DATASET_FOLDER, 'X_train.ftr'), load_ftr(DATASET_FOLDER, 'X_test.ftr')
y_train, y_test = load_ftr(DATASET_FOLDER, 'y_train.ftr'), load_ftr(DATASET_FOLDER, 'y_test.ftr')

from imblearn.under_sampling import TomekLinks

tl = TomekLinks(sampling_strategy='auto').fit_resample(X_train, y_train)
